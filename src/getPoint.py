...
class PanZoomWindow(object):
    ...
    def onMouse(self, event, x, y, _ignore1, _ignore2):
        ...
        if event == cv2.EVENT_MOUSEMOVE:
            ...
            if np.any(coordsInDisplayedImage < 0) or np.any(coordsInDisplayedImage > self.panAndZoomState.shape[:2]):
                ...
                ws.cell(column=2, row=newRowLocation, value=coordsInFullImage[0])
                ws.cell(column=3, row=newRowLocation, value=coordsInFullImage[1])
                ws.cell(column=4, row=newRowLocation, value=picName)
    ...
    def redrawImage(self):
        ...
        cv2.imshow(self.WINDOW_NAME,
                   self.img[int(pzs.ul[0]):int(pzs.ul[0] + pzs.shape[0]), int(pzs.ul[1]):int(pzs.ul[1] + pzs.shape[1])])
...
if __name__ == "__main__":
    ...
    while key != ord('q'):  # 27 = escape key
        ...
        window = PanZoomWindow(myImage, "Labeling")
        if key == ord('a') and i > 0:
            i -= 1
            print(str(i) + "====>")
        elif key == ord('d') and i < len(image_names) - 1:
            i = i + 1
            print(str(i) + "====>")
